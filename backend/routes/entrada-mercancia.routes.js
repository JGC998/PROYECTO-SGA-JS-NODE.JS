...
router.post('/entrada-mercancia', async (req, res) => {
    try {
        ...
        // Verificar que la ubicación existe y obtener el código exacto (con espacios) de la BD
        const ubiExiste = await q(pool).input('ubi', ubiStr)
            .query('SELECT UBICODUBI FROM UBICACION WHERE RTRIM(UBICODUBI) = @ubi');
        if (!ubiExiste.recordset.length)
            return res.status(404).json({ error: `Ubicación ${ubiStr} no encontrada` });
        const ubiExacta = ubiExiste.recordset[0].UBICODUBI; // conserva el padding original
        ...
        // Actualizar STOCK y STOCKLOTE manualmente (pr_grabarCompraDirecta graba en ALBARANCS
        // pero no actualiza las tablas de stock consolidado)
        await q(pool)
            .input('art', artStr).input('ubi', ubiExacta).input('lot', lotStr).input('cant', cant)
            .query(`
                IF EXISTS (SELECT 1 FROM STOCKLOTE WHERE RTRIM(STOARTCOD)=@art AND STOUBI=@ubi AND RTRIM(STOLOT)=@lot)
                    UPDATE STOCKLOTE SET STOCAN = STOCAN + @cant
                    WHERE RTRIM(STOARTCOD)=@art AND STOUBI=@ubi AND RTRIM(STOLOT)=@lot
                ELSE
                    INSERT INTO STOCKLOTE (STOARTCOD, STOUBI, STOLOT, STO2LOT, STOCAN)
                    VALUES (@art, @ubi, @lot, @lot, @cant)
            `);
        await q(pool)
            .input('art', artStr).input('ubi', ubiExacta).input('lot', lotStr).input('cant', cant)
            .query(`
                IF EXISTS (SELECT 1 FROM STOCK WHERE RTRIM(STOARTCOD)=@art AND STOUBI=@ubi AND RTRIM(STOLOT)=@lot)
                    UPDATE STOCK SET STOCAN = STOCAN + @cant
                    WHERE RTRIM(STOARTCOD)=@art AND STOUBI=@ubi AND RTRIM(STOLOT)=@lot
                ELSE
                    INSERT INTO STOCK (STOARTCOD, STOUBI, STOLOT, STOCAN)
                    VALUES (@art, @ubi, @lot, @cant)
            `);
