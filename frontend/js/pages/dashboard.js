...
(function () {
    ...
    function fillDays(data, keyX, keyY, desde, hasta) {
        var map = {};
        data.forEach(function (r) { map[String(r[keyX]).slice(0, 10)] = r[keyY] || 0; });
    ...
    function renderSparkline(containerId, data, keyX, keyY, desde, hasta) {
        var el = document.getElementById(containerId);
        if (!el) return;
        if (!data || !data.length) { el.innerHTML = '<div class="db-empty">Sin datos</div>'; return; }

        if (desde && hasta) data = fillDays(data, keyX, keyY, desde, hasta);

        var max = Math.max.apply(null, data.map(function (r) { return r[keyY] || 0; })) || 1;

        var lastActive = data.reduce(function (acc, r) {
            return (r[keyY] || 0) > 0 ? r : acc;
        }, data[data.length - 1]);

        var tooltip = document.createElement('div');
        tooltip.className = 'db-spark-tooltip db-spark-tooltip--visible';
        tooltip.textContent = String(lastActive[keyX] || '') + ': ' + fmt(lastActive[keyY] || 0) + ' mov.';
        ...
        data.forEach(function (r) {
            var val = r[keyY] || 0;
            var h = Math.max(Math.round((val / max) * 100), 3);

            var wrapper = document.createElement('div');
            wrapper.className = 'db-spark-col';

            var label = document.createElement('span');
            label.className = 'db-spark-label';
            label.textContent = val > 0 ? fmt(val) : '';

            var bar = document.createElement('div');
            bar.className = 'db-spark-bar' + (val === 0 ? ' db-spark-bar--empty' : '');
            bar.style.height = (val === 0 ? '3px' : h + '%');

            wrapper.addEventListener('mouseenter', function () {
                tooltip.textContent = String(r[keyX] || '') + ': ' + fmt(val) + ' mov.';
            });
        ...
        el.innerHTML = '';
        el.append(tooltip, barsDiv, labelsDiv);
    }
