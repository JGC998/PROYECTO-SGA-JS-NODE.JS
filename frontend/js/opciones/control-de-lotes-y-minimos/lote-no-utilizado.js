...
async function cargarDatos() {
    ...
    try {
        const data = await SGA.loteNoUtilizado.list(params);
        renderTabla(data);
    } catch {
        console.error('Error al cargar lote no utilizado');
    }
}

function renderTabla(rows) {
    const tbody = document.getElementById('tbody-lnoutilizado');
...
function agregarFila() {
    ...
    tr.innerHTML = `
        ...
        <td><input type="text" class="cell-input" readonly></td>
        <td><input type="text" class="cell-input" placeholder="Lote a excluir"></td>`;
    tbody.appendChild(tr);
    tr.querySelector('input').focus();
}

async function guardarCambios() {
    const filas = [...document.querySelectorAll('#tbody-lnoutilizado .edit-row')].map(tr => {
        const inputs = tr.querySelectorAll('input');
        return { cliente: inputs[0].value, articulo: inputs[2].value, lote_exclusivo: inputs[4].value };
    }).filter(f => f.cliente || f.articulo);
    try {
        await SGA.loteNoUtilizado.save(filas);
