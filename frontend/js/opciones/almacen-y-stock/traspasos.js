...
(function () {
    ...
    function renderLoading() {
        setStockHint('Consultando stock…');
    ...
    function setStockHint(msg) {
        var h = getEl('tp-stock-hint');
    ...
    function renderStockList(rows) {
        ...
        if (!rows || !rows.length) {
            setStockHint('Sin stock disponible');
        ...
        setStockHint(rows.length + ' línea(s) con stock');
    ...
    function buildStockCard(row, idx) {
        var cant = Number(row.stock != null ? row.stock : (row.cantidad != null ? row.cantidad : 0));
    ...
    function selectOrigen(idx) {
        ...
        _selectedOrigen = {
            ...
            lot:       row.lote || '',
            disponible: Number(row.stock != null ? row.stock : (row.cantidad != null ? row.cantidad : 0))
        };
    ...
    function buscarUbiDestino() {
        var cod = val('tp-dest-input');
        var pill = getEl('tp-dest-pill');
        if (!pill) return;
        if (!cod) { pill.hidden = true; pill.textContent = ''; return; }

        pill.hidden = false;
        pill.className = 'tp-dest-pill tp-dest-pill--loading';
        ...
        SGA.ubicaciones.list({ ubicacion: cod })
            .then(function (data) {
                ...
                if (found) {
                    var label = '📍 ' + (found.ubicacion || cod).trim();
                    if (found.nombre || found.nom_ubicacion) label += ' · ' + (found.nombre || found.nom_ubicacion);
                    pill.className = 'tp-dest-pill tp-dest-pill--ok';
                    pill.textContent = label;
                } else {
                    pill.className = 'tp-dest-pill tp-dest-pill--new';
                    pill.textContent = '+ Ubicación nueva — se creará al confirmar';
                }
            })
            .catch(function () {
                pill.className = 'tp-dest-pill tp-dest-pill--new';
    ...
    function stepCant(delta) {
        var inp = getEl('tp-qty-input');
        if (!inp) return;
        var curr = parseFloat(inp.value) || 0;
        var max  = _selectedOrigen ? _selectedOrigen.disponible : Infinity;
        var next = Math.max(1, Math.min(max, curr + delta));
        inp.value = (next % 1 === 0) ? String(Math.round(next)) : next.toFixed(3);
        var e = getEl('tp-qty-error');
        if (e) e.textContent = '';
        inp.classList.remove('tp-qty-input--error');
    }
    ...
    function resetDestForm() {
        ...
        var pill = getEl('tp-dest-pill');
        if (pill) { pill.hidden = true; pill.textContent = ''; pill.className = 'tp-dest-pill'; }
    }
    ...
    function resetAll() {
        ...
        setStockHint('Busca un artículo primero');
        var section = getEl('tp-stock-section');
        if (section) {
            section.innerHTML = '';
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        // Steppers cantidad
        var btnMenos = getEl('tp-btn-menos');
        var btnMas   = getEl('tp-btn-mas');
        if (btnMenos) btnMenos.addEventListener('click', function () { stepCant(-1); });
        if (btnMas)   btnMas.addEventListener('click',   function () { stepCant(+1); });
        ...
        var destInput = getEl('tp-dest-input');
        if (destInput) {
            destInput.addEventListener('blur', buscarUbiDestino);
            destInput.addEventListener('keydown', function (e) {
                if (e.key === 'Enter') { e.preventDefault(); buscarUbiDestino(); }
            });
