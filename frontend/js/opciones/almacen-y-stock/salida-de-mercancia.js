...
(function () {
    ...
    function renderCarrito() {
        ...
        // Remove only the line cards, not the persistent empty-state element
        var child = body.firstChild;
        while (child) {
            var next = child.nextSibling;
            if (!child.id || child.id !== 'sm-carrito-empty') body.removeChild(child);
            child = next;
        }
    ...
    function buildLineaCarritoEl(l) {
        var div = el('div', 'sm-line');
        div.dataset.id = l.id;

        var info = el('div', 'sm-line-info');

        var artRow  = el('div', 'sm-line-art-row');
        var artSpan = el('span', 'sm-line-art');
        artSpan.appendChild(txt(l.cod));
        artRow.appendChild(artSpan);
        if (l.nombre && l.nombre !== l.cod) {
            var nomSpan = el('span', 'sm-line-nombre');
            nomSpan.appendChild(txt(l.nombre));
            artRow.appendChild(nomSpan);
        }
        info.appendChild(artRow);

        var tags = el('div', 'sm-line-tags');
        var lotBadge = el('span', 'sm-line-lote');
        lotBadge.appendChild(txt(lotLabel(l.lot)));
        tags.appendChild(lotBadge);
        var ubiBadge = el('span', 'sm-line-ubi');
        ubiBadge.appendChild(txt(l.ubi + (l.ubiNombre ? ' · ' + l.ubiNombre : '')));
        tags.appendChild(ubiBadge);
        info.appendChild(tags);

        div.appendChild(info);

        var right    = el('div', 'sm-line-right');
        var cantSpan = el('span', 'sm-line-cant');
        cantSpan.appendChild(txt(fmt(l.cant)));
        right.appendChild(cantSpan);

        var btnDel = el('button', 'sm-line-del');
        btnDel.type = 'button';
        btnDel.setAttribute('aria-label', 'Eliminar línea');
        btnDel.appendChild(txt('🗑'));
        (function (id) {
            btnDel.addEventListener('click', function () { eliminarLinea(id); });
        }(l.id));
    ...
    async function saveLinea(l) {
        return SGA.salidas.save({ cod: l.cod, ubi: l.ubi, lot: l.lot, cant: l.cant })
            .catch(function (err) {
                ...
                if (match) {
                    ...
                    if (status === 404) throw new Error('Artículo o ubicación no encontrados');
                    if (status === 409) throw new Error('Stock insuficiente en ubicación');
    ...
    function renderResultados(snap, results, nOk, nErr) {
        ...
        var title = $('sm-result-title');
        if (title) {
            title.textContent = nErr === 0
                ? 'Salida completada'
                : 'Salida — ' + nOk + ' de ' + total + ' líneas registradas';
        }

        var body = $('sm-result-modal-body');
        body.innerHTML = '';
        ...
        snap.forEach(function (l, i) {
            ...
            var row = el('div',
                ok ? 'sm-result-line sm-result-line--ok' : 'sm-result-line sm-result-line--err'
            );

            var iconSpan = el('span', 'sm-result-line-icon');
            iconSpan.appendChild(txt(ok ? '✓' : '✗'));
            row.appendChild(iconSpan);

            var infoSpan = el('span', 'sm-result-line-info');
            if (ok) {
                var d     = results[i].value;
                var alb   = d.serie + '/' + d.albaran;
                var delta = 'Stock: ' + fmt(d.stock_antes) + ' → ' + fmt(d.stock_nuevo);
        ...
        if (firstOkIdx !== -1) {
            $('link-ver-stock').href =
                ...
                encodeURIComponent(snap[firstOkIdx].cod);
        }

        $('sm-result-overlay').classList.add('sm-result-overlay--active');
    }

    function closeResultModal() {
        var overlay = $('sm-result-overlay');
        if (overlay) overlay.classList.remove('sm-result-overlay--active');
    }
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        /* Artículo */
        $('sm-art-cod').addEventListener('blur', function () {
            if (_artState === 'found') return;
            buscarArticulo();
        });
        ...
        $('btn-art-buscar').addEventListener('mousedown', function (e) { e.preventDefault(); });
        $('btn-art-buscar').addEventListener('click', function () { buscarArticulo(); });
        ...
        $('sm-result-close').addEventListener('click', closeResultModal);
        $('sm-result-overlay').addEventListener('click', function (e) {
