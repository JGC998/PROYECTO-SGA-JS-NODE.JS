...
async function cargarProveedores() {
    ...
    tbody.innerHTML = '<tr><td colspan="9" class="loading">Cargando proveedores...</td></tr>';
    try {
        todosProveedores = await SGA.proveedores.list();
        renderTabla(todosProveedores);
    } catch {
        tbody.innerHTML = '<tr><td colspan="9" class="loading">Error al conectar con el servidor.</td></tr>';
    }
}
...
function renderTabla(rows) {
    const tbody = document.getElementById('tbody-proveedores');
    if (!rows.length) {
        tbody.innerHTML = '<tr><td colspan="9" class="loading">No se encontraron proveedores.</td></tr>';
        return;
    }
    tbody.innerHTML = rows.map((r, i) => `
        <tr data-idx="${i}">
            ...
            <td style="white-space:nowrap">
                ...
                <button class="btn-icon btn-albaranes" data-idx="${i}" title="Ver entradas">📋</button>
            </td>
        </tr>`).join('');
    ...
    tbody.querySelectorAll('.btn-albaranes').forEach(btn => {
        btn.addEventListener('click', function () {
            const idx = parseInt(this.dataset.idx);
            verAlbaranes(rows[idx]);
        });
    });
}

function editarProveedor(r) {
    ...
    const campo = (label, id, val) => `
        <div style="margin-bottom:14px">
            ...
            <input id="prov-${id}" type="text" value="${(val || '').trim().replace(/"/g, '&quot;')}"
    ...
    modal.innerHTML = `
        <div style="font-weight:700;font-size:1.05rem;margin-bottom:4px;color:#1e3a5f">Editar proveedor</div>
        ...
        <div id="prov-error" style="display:none;color:#dc2626;font-size:.82rem;margin-bottom:10px"></div>
        <div style="display:flex;gap:8px;justify-content:flex-end;margin-top:8px">
            <button id="prov-cancel" style="padding:9px 18px;border:1px solid #d1d5db;border-radius:6px;background:#fff;cursor:pointer;font-size:.85rem">Cancelar</button>
            <button id="prov-save" style="padding:9px 20px;border:none;border-radius:6px;background:#2563c0;color:#fff;font-weight:700;cursor:pointer;font-size:.85rem">💾 Guardar</button>
        </div>`;
    ...
    modal.querySelector('#prov-cancel').addEventListener('click', cerrar);
    overlay.addEventListener('click', e => { if (e.target === overlay) cerrar(); });

    modal.querySelector('#prov-save').addEventListener('click', async () => {
        const btn = modal.querySelector('#prov-save');
        ...
        const datos = {
            nombre:       modal.querySelector('#prov-nombre').value.trim(),
            razon_social: modal.querySelector('#prov-razon_social').value.trim(),
            direccion:    modal.querySelector('#prov-direccion').value.trim(),
            localidad:    modal.querySelector('#prov-localidad').value.trim(),
            cif:          modal.querySelector('#prov-cif').value.trim(),
            telefono:     modal.querySelector('#prov-telefono').value.trim(),
            contacto:     modal.querySelector('#prov-contacto').value.trim(),
            email:        modal.querySelector('#prov-email').value.trim(),
        };

        try {
            await SGA.proveedores.update(r.codigo.trim(), datos);
...
function verAlbaranes(r) {
    ...
    modal.innerHTML = `
        <div style="font-weight:700;font-size:1.05rem;color:#1e3a5f;margin-bottom:2px">Entradas del proveedor</div>
    ...
    SGA.movimientos.list({ cliente: cod, pageSize: 200 })
        .then(function (data) {
            const rows = (Array.isArray(data) ? data : []).filter(m => m.tipo === 'PP' || m.tipo === 'RG');
            const tabla = modal.querySelector('#alb-tabla');
            if (!rows.length) {
                tabla.innerHTML = '<div style="text-align:center;padding:24px;color:#9ca3af;font-size:.85rem">Sin entradas registradas para este proveedor.</div>';
                return;
            }
            tabla.innerHTML = `<table style="width:100%;border-collapse:collapse;font-size:.82rem">
                ...
                <tbody>
                    ${rows.map(m => `<tr style="border-bottom:1px solid #f3f4f6">
                        ...
                        <td style="padding:8px 12px;text-align:right;color:#059669;font-weight:700">+${m.cantidad || 0}</td>
                    </tr>`).join('')}
                </tbody>
            </table>`;
        })
        .catch(function () {
            modal.querySelector('#alb-tabla').innerHTML = '<div style="text-align:center;padding:24px;color:#dc2626;font-size:.85rem">Error al cargar las entradas.</div>';
        });
}
