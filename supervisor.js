...
(function () {
    ...
    async function loadWorkers() {
        try {
            const workers = await fetch('/js/shared/workers.json').then(r => r.json());
    ...
    async function loadArticles() {
        try {
            const r = await fetch('/datos/articulos.json');
            if (!r.ok) return;
    ...
    async function deletePicking(id) {
        ...
        try {
            const r = await fetch(`/picking/${id}`, { method: 'DELETE' });
    ...
    function showQR(routeId) {
        ...
        document.getElementById('qr-img').src = `/qr?data=${encodeURIComponent(mobileUrl)}`;
    ...
    async function exportPDF(routeId, docNum, paradas, workerName) {
        ...
        if (mobileUrl) {
            try {
                const r = await fetch(`/qr?data=${encodeURIComponent(mobileUrl)}`);
                if (r.ok) {
            ...
            es.addEventListener('message', e => {
                try {
                    ...
                    if (msg.type === 'ruta-progreso') {
                        const picking = _pickings.find(p => p.id === msg.id);
                        if (picking) {
                            if (picking.paradas[msg.idx]) {
                                picking.paradas[msg.idx].completada = true;
                                picking.paradas[msg.idx].timestamp  = new Date().toISOString();
                                if (msg.qtyCounted != null) picking.paradas[msg.idx].qtyCounted = msg.qtyCounted;
                            }
                            if (msg.completadas === msg.total) picking.estado = 'completada';
                            renderStats();
                            renderMonitor();
                        } else {
                            loadPickings();
                        }
                    } else if (msg.type === 'incidencia') {
                        const picking = _pickings.find(p => p.id === msg.id);
                        if (picking?.paradas[msg.stopIdx]) {
                            if (!picking.paradas[msg.stopIdx].incidencias) picking.paradas[msg.stopIdx].incidencias = [];
                            picking.paradas[msg.stopIdx].incidencias.push({ texto: msg.texto, tipo: msg.tipo, ts: msg.ts });
                            renderMonitor();
                            showToast(`⚠ Incidencia en ${esc(msg.locKey)}${msg.operario ? ' · ' + esc(msg.operario) : ''}`, 5000);
                        } else { loadPickings(); }
